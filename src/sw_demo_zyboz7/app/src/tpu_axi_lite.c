...
u32 tpu_cycles(void)
{
    return tpu_read_reg(TPU_REG_CYCLES) & 0x0000FFFFU;
}

u32 tpu_debug0(void)
{
    return tpu_read_reg(TPU_REG_DEBUG0);
}

int tpu_wait_ready(u32 timeout_polls, u32 *final_status)
{
    u32 status = 0U;
    ...
    while (timeout_polls != 0U) {
        ...
        if ((status & (TPU_STATUS_BUSY | TPU_STATUS_TOP_DONE)) == 0U) {
            if (final_status != 0) {
                *final_status = status;
            }
            return 0;
...
int tpu_start(void)
{
    ...
    return ((tpu_status() & TPU_STATUS_CMD_ERROR) == 0U) ? 0 : -1;
}
...
int tpu_wait_done(u32 timeout_polls, u32 *final_status)
{
    ...
    while (timeout_polls != 0U) {
        status = tpu_status();
        if ((status & (TPU_STATUS_ERROR | TPU_STATUS_CMD_ERROR | TPU_STATUS_OVERFLOW)) != 0U) {
            if (final_status != 0) {
                *final_status = status;
            }
            return -1;
...
int tpu_ub_read(u32 bank, u32 addr, s8 *value)
{
    ...
    while (timeout != 0U) {
        ...
        if ((status & TPU_STATUS_UB_RD_VALID) != 0U) {
