...
module test_controller_v3_rom_spatial_block_conv1;
  ...
  localparam logic [15:0] SPATIAL_BASE = 16'd161;
  localparam logic [15:0] SPATIAL_COUNT = 16'd2;
  localparam logic [15:0] OC_TILE = 16'd0;
  ...
  logic [4:0] controller_block_state_o;
  logic [15:0] controller_k_tile_o;
  logic [15:0] controller_block_row_o;
  ...
  tpu_controller_v3_rom_spatial_block #(
      .SIZE               (SIZE),
      .DATA_WIDTH         (DATA_WIDTH),
      .OUT_WIDTH          (OUT_WIDTH),
      ...
      .dbg_block_state_o                (controller_block_state_o),
      .dbg_k_tile_o                     (controller_k_tile_o),
      .dbg_block_row_o                  (controller_block_row_o),
      ...
      .spatial_base_idx_i               (SPATIAL_BASE),
      .spatial_count_i                  (SPATIAL_COUNT),
      .oc_tile_idx_i                    (OC_TILE),
  ...
  task automatic preload_input_image();
    ...
    $display("  Conv1 spatial block: spatial_base=%0d count=%0d oc_tile=%0d",
             SPATIAL_BASE, SPATIAL_COUNT, OC_TILE);
  endtask

  task automatic run_conv1_spatial_block();
    int cycles;

    print_header("TEST: V3 ROM spatial-block controller -> Conv1 oc_tile0");
    host_mode = 1'b0;
    ...
    while (!controller_done_o && !controller_error_o && (cycles < TIMEOUT_CYCLES)) begin
      ...
      if (act_req_valid_w && act_req_ready_w) begin
        act_req_count++;
        if (act_req_count <= 6) begin
          $display("  ACT REQ: k_tile=%0d block_row=%0d addr={lane1:%0d,lane0:%0d}",
                   controller_k_tile_o, controller_block_row_o,
                   act_req_addr_flatten_w[(1*UB_ADDR_WIDTH)+:UB_ADDR_WIDTH],
                   act_req_addr_flatten_w[(0*UB_ADDR_WIDTH)+:UB_ADDR_WIDTH]);
        end
      end
      ...
      if (accumulator_read_valid_w) begin
        ...
        if (acc_read_count == 0) begin
          expect_i32_eq("conv1 spatial161 acc oc0",
                        accumulator_read_flatten_w[(0*ACC_WIDTH)+:ACC_WIDTH],
                        32'sd9485);
          expect_i32_eq("conv1 spatial161 acc oc1",
      ...
      if (vpu_data_valid_w) begin
        vpu_output_count++;
        $display("  VPU OUT[%0d]: data={lane1:%0d,lane0:%0d}",
                 vpu_output_count - 1,
                 $signed(vpu_data_flatten_w[(1*OUT_WIDTH)+:OUT_WIDTH]),
                 $signed(vpu_data_flatten_w[(0*OUT_WIDTH)+:OUT_WIDTH]));
      end

      if (ctrl_ub_wr_en_w) begin
        ub_write_count++;
        $display("  UB WRITE[%0d]: bank%0d[%0d] = %0d",
                 ub_write_count - 1, ctrl_ub_wr_bank_w, ctrl_ub_wr_addr_w, $signed(ctrl_ub_wr_data_w));
      end
    end

    $display("  CTRL   : done=%b error=%b state=%0d block_state=%0d k_tile=%0d block_row=%0d cycles=%0d err=0x%08h",
             controller_done_o, controller_error_o, controller_state_o, controller_block_state_o,
             controller_k_tile_o, controller_block_row_o,
    ...
    expect_i32_eq("weight request count", wgt_req_count, 5);
    expect_i32_eq("activation request count", act_req_count, 10);
    expect_i32_eq("accumulator read count", acc_read_count, 2);
    expect_i32_eq("VPU output count", vpu_output_count, 2);
    expect_i32_eq("UB write count", ub_write_count, 4);
  endtask
  ...
  task automatic check_output_buffer();
    ...
    print_header("TEST: Host readback Conv1 spatial-block outputs");
    host_mode = 1'b1;
    mismatches = 0;

    for (int row = 0; row < SPATIAL_COUNT; row++) begin
      for (int oc = 0; oc < SIZE; oc++) begin
        int spatial;
        spatial = int'(SPATIAL_BASE) + row;
        addr = OUT_BASE + UB_ADDR_WIDTH'(oc * CONV1_NUM_SPATIAL) + UB_ADDR_WIDTH'(spatial);
        expected = conv1_out_mem[(oc * CONV1_NUM_SPATIAL) + spatial];
        host_read(1'b1, addr, actual);
        $display("  out[oc%0d spatial%0d] = %0d expected=%0d",
                 oc, spatial, $signed(actual), $signed(expected));
        expect_i8_eq($sformatf("conv1 out oc%0d spatial%0d", oc, spatial), actual, expected);
        if (actual !== expected) begin
          mismatches++;
        end
      end
  ...
  initial begin
    ...
    preload_input_image();
    run_conv1_spatial_block();
    check_output_buffer();

    $display("");
    $display("DEBUG COUNTERS");
    $display("  ctrl_cycles=%0d final_state=%0d final_k_tile=%0d final_block_row=%0d",
             controller_cycle_count_o, controller_state_o, controller_k_tile_o, controller_block_row_o);
    ...
    $display("test_controller_v3_rom_spatial_block_conv1: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_v3_rom_spatial_block_conv1 failed");
    end
    $finish;
  end

endmodule : test_controller_v3_rom_spatial_block_conv1
