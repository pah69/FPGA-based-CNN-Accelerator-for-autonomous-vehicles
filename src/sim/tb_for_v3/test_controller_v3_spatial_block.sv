...
module test_controller_v3_spatial_block;
  ...
  localparam int BLOCK_SIZE = 2;
  localparam int CLK_PERIOD = 10;
  localparam int MAX_RUN_CYCLES = 700;
  ...
  logic [15:0] ctrl_dbg_block_row_o;
  ...
  logic block_output_valid_w;
  logic [15:0] block_output_spatial_idx_w;
  logic [15:0] block_output_row_idx_w;
  ...
  int act_req_count;
  int acc_read_count;
  int block_output_count;
  logic seen_ctrl_done;

  tpu_controller_v3_spatial_block #(
      .SIZE               (SIZE),
      ...
      .dbg_block_row_o                    (ctrl_dbg_block_row_o),
      ...
      .k_total_i                          (16'd4),
      .num_k_tiles_i                      (16'd2),
      .spatial_base_idx_i                 (16'd0),
      .spatial_count_i                    (16'd2),
      .oc_tile_idx_i                      (16'd0),
      ...
      .block_output_valid_o               (block_output_valid_w),
      .block_output_spatial_idx_o         (block_output_spatial_idx_w),
      .block_output_row_idx_o             (block_output_row_idx_w),
  ...
  task automatic check_activation_req(input int req_idx);
    begin
      if (req_idx == 0 || req_idx == 1) begin
        expect_i32($sformatf("act_req%0d lane0 addr", req_idx), act_req_addr(0), 20);
        expect_i32($sformatf("act_req%0d lane1 addr", req_idx), act_req_addr(1), 21);
      end else begin
        expect_i32($sformatf("act_req%0d lane0 addr", req_idx), act_req_addr(0), 22);
        expect_i32($sformatf("act_req%0d lane1 addr", req_idx), act_req_addr(1), 23);
      end
      expect_flag($sformatf("act_req%0d valid mask", req_idx), act_req_lane_valid_w == 2'b11);
      expect_flag($sformatf("act_req%0d zero mask", req_idx), act_req_lane_zero_w == 2'b00);
    end
  endtask
  ...
  initial begin
    ...
    print_header("TEST: V3 spatial-block controller -> TPU datapath");
    @(negedge clk);
    start_i = 1'b1;
    @(posedge clk);
    @(negedge clk);
    ...
    for (int cycle = 0; cycle < MAX_RUN_CYCLES; cycle++) begin
      ...
      if (wgt_req_valid_w && wgt_req_ready_w) begin
        $display("  WGT REQ[%0d]: req_k_tile=%0d block_row=%0d addr={{r1c1:%0d,r1c0:%0d},{r0c1:%0d,r0c0:%0d}}",
                 wgt_req_count, int'(wgt_req_tag_w - 16'h5000), ctrl_dbg_block_row_o,
      ...
      if (act_req_valid_w && act_req_ready_w) begin
        $display("  ACT REQ[%0d]: k_tile=%0d block_row=%0d addr={lane1:%0d,lane0:%0d}",
                 act_req_count, ctrl_dbg_k_tile_o, ctrl_dbg_block_row_o,
                 act_req_addr(1), act_req_addr(0));
        check_activation_req(act_req_count);
        act_req_count++;
      end

      if (act_launch_w && act_launch_ready_w) begin
        $display("  ACT LAUNCH: k_tile=%0d block_row=%0d", ctrl_dbg_k_tile_o, ctrl_dbg_block_row_o);
      end

      if (mxu_psum_valid_o[0]) begin
        $display("  CAPTURE: k_tile=%0d row=%0d MXU psum lane0=%0d",
                 ctrl_dbg_k_tile_o, ctrl_dbg_block_row_o, $signed(get_mxu_psum(0)));
      end

      if (mxu_psum_valid_o[1]) begin
        $display("  CAPTURE: k_tile=%0d row=%0d MXU psum lane1=%0d",
                 ctrl_dbg_k_tile_o, ctrl_dbg_block_row_o, $signed(get_mxu_psum(1)));
      end

      if (accumulator_row_done_o) begin
        $display("  ACC DONE: row=%0d ready=%b",
                 accumulator_row_done_addr_o, accumulator_row_ready_o[accumulator_row_done_addr_o]);
      end

      if (accumulator_read_valid_o) begin
        $display("  ACC READ[%0d]: row=%0d data={lane1:%0d,lane0:%0d}",
                 acc_read_count, accumulator_read_addr_w,
                 $signed(get_acc_lane(1)), $signed(get_acc_lane(0)));
        expect_i32($sformatf("acc row%0d lane0 sum", acc_read_count), $signed(get_acc_lane(0)), 46);
        expect_i32($sformatf("acc row%0d lane1 sum", acc_read_count), $signed(get_acc_lane(1)), -65);
        acc_read_count++;
      end

      if (block_output_valid_w) begin
        $display("  BLOCK OUT[%0d]: spatial=%0d row=%0d data={lane1:%0d,lane0:%0d}",
                 block_output_count, block_output_spatial_idx_w, block_output_row_idx_w,
                 $signed(get_vpu_lane(1)), $signed(get_vpu_lane(0)));
        expect_i32($sformatf("block output%0d spatial", block_output_count),
                   block_output_spatial_idx_w, block_output_count);
        expect_i32($sformatf("block output%0d row", block_output_count),
                   block_output_row_idx_w, block_output_count);
        expect_i32($sformatf("block output%0d lane0", block_output_count), $signed(get_vpu_lane(0)), 50);
        expect_i32($sformatf("block output%0d lane1", block_output_count), $signed(get_vpu_lane(1)), 0);
        block_output_count++;
      end
    ...
    expect_i32("activation request count", act_req_count, 4);
    expect_i32("accumulator read count", acc_read_count, 2);
    expect_i32("block output count", block_output_count, 2);
    expect_i32("activation vectors pushed", int'(dbg_act_vectors_pushed_o), 4);
    expect_i32("activation lane reads", int'(dbg_act_lane_reads_o), 8);
    expect_i32("weight stream reuse count", int'(dbg_weight_reuse_count_o), 2);

    $display("");
    $display("DEBUG COUNTERS");
    $display("  ctrl_cycles=%0d final_state=%0d final_k_tile=%0d final_block_row=%0d",
             ctrl_dbg_cycle_count_o, ctrl_dbg_state_o, ctrl_dbg_k_tile_o, ctrl_dbg_block_row_o);
    ...
    $display("test_controller_v3_spatial_block: checks=%0d pass=%0d fail=%0d",
             checks, passes, fails);
    if (fails != 0) begin
      $fatal(1, "test_controller_v3_spatial_block failed");
    end
    $finish;
  end

endmodule : test_controller_v3_spatial_block
