...
module test_controller_v3_conv2_layer;
  ...
  localparam int TIMEOUT_CYCLES = 700000;
  ...
  localparam int CONV2_OUTPUT_PAIRS = (CONV2_C / SIZE) * CONV2_H * CONV2_W;
  ...
  logic signed [DATA_WIDTH-1:0] pool1_mem[0:POOL1_OUT_COUNT-1];
  ...
  task automatic build_pool1_input();
    int in_base;
    int out_idx;
    ...
    for (int c = 0; c < CONV1_C; c++) begin
      for (int oh = 0; oh < POOL1_H; oh++) begin
        for (int ow = 0; ow < POOL1_W; ow++) begin
          ...
          pool1_mem[out_idx] = max_i8(max_i8(p00, p01), max_i8(p10, p11));
        end
      end
    end
  endtask

  task automatic preload_pool1_input();
    print_header("TEST: Host preload Pool1 tensor");
    host_mode = 1'b1;
    ...
    for (int idx = 0; idx < POOL1_OUT_COUNT; idx++) begin
      host_write(1'b0, UB_ADDR_WIDTH'(idx), pool1_mem[idx]);
    end

    $display("  UB bank0[0:%0d] loaded with derived Pool1 tensor", POOL1_OUT_COUNT - 1);
    $display("  Conv2 full layer: output count=%0d channel-major layer1_out_i8.hex",
             CONV2_OUT_COUNT);
  endtask

  task automatic run_conv2_layer();
    int cycles;
    int output_pairs;

    print_header("TEST: V3 ROM layer controller -> full Conv2 output tensor");
    host_mode = 1'b0;
    ...
    while (!controller_done_o && !controller_error_o && (cycles < TIMEOUT_CYCLES)) begin
      ...
      if (accumulator_read_valid_w) begin
        ...
        if ((captured_acc_count % 128) == 0) begin
    ...
    expect_flag("all Conv2 oc/spatial tiles completed", controller_tile_count_o == 32'(CONV2_OUTPUT_PAIRS));
    expect_flag("all Conv2 VPU pairs observed", vpu_output_count == CONV2_OUTPUT_PAIRS);
    expect_flag("all Conv2 accumulator reads observed", captured_acc_count == CONV2_OUTPUT_PAIRS);
  endtask
  ...
  task automatic check_output_buffer();
    ...
    print_header("TEST: Host readback full Conv2 output tensor");
    host_mode = 1'b1;
    mismatches = 0;

    for (int idx = 0; idx < CONV2_OUT_COUNT; idx++) begin
      addr = OUT_BASE + UB_ADDR_WIDTH'(idx);
      expected = conv2_out_mem[idx];
      host_read(1'b1, addr, actual);

      expect_i8_eq($sformatf("conv2_out[%0d]", idx), actual, expected);
      if (actual !== expected) begin
        mismatches++;
      end
    end

    $display("  samples: out[0]=%0d out[120]=%0d out[241]=%0d out[%0d]=%0d",
             $signed(conv2_out_mem[0]), $signed(conv2_out_mem[120]),
             $signed(conv2_out_mem[241]), CONV2_OUT_COUNT - 1,
             $signed(conv2_out_mem[CONV2_OUT_COUNT - 1]));
  ...
  initial begin
    ...
    $display("test_controller_v3_conv2_layer: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_v3_conv2_layer failed");
    end
    $finish;
  end

endmodule : test_controller_v3_conv2_layer
