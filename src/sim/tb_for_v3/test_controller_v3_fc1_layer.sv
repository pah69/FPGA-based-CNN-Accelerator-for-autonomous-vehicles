...
module test_controller_v3_fc1_layer;
  ...
  localparam int CONV2_C = 10;
  localparam int CONV2_H = 11;
  localparam int CONV2_W = 11;
  localparam int POOL2_H = 5;
  localparam int POOL2_W = 5;
  localparam int FC1_OUT_COUNT = 16;
  localparam int CONV2_OUT_COUNT = CONV2_C * CONV2_H * CONV2_W;
  localparam int POOL2_OUT_COUNT = CONV2_C * POOL2_H * POOL2_W;
  localparam int FC1_OUTPUT_PAIRS = FC1_OUT_COUNT / SIZE;
  ...
  logic signed [DATA_WIDTH-1:0] conv2_out_mem[0:CONV2_OUT_COUNT-1];
  logic signed [DATA_WIDTH-1:0] pool2_mem[0:POOL2_OUT_COUNT-1];
  logic signed [DATA_WIDTH-1:0] fc1_out_mem[0:FC1_OUT_COUNT-1];
  ...
  task automatic build_pool2_input();
    int in_base;
    int out_idx;
    ...
    for (int c = 0; c < CONV2_C; c++) begin
      for (int oh = 0; oh < POOL2_H; oh++) begin
        for (int ow = 0; ow < POOL2_W; ow++) begin
          in_base = (c * CONV2_H * CONV2_W) + ((oh * 2) * CONV2_W) + (ow * 2);
          out_idx = (c * POOL2_H * POOL2_W) + (oh * POOL2_W) + ow;
          p00 = conv2_out_mem[in_base];
          p01 = conv2_out_mem[in_base + 1];
          p10 = conv2_out_mem[in_base + CONV2_W];
          p11 = conv2_out_mem[in_base + CONV2_W + 1];
          pool2_mem[out_idx] = max_i8(max_i8(p00, p01), max_i8(p10, p11));
        end
      end
    end
  endtask

  task automatic preload_pool2_input();
    print_header("TEST: Host preload Pool2 tensor");
    host_mode = 1'b1;
    $readmemh("../../../CNN_model/python/mnist_classification/18_05/layer1_out_i8.hex",
              conv2_out_mem);
    $readmemh("../../../CNN_model/python/mnist_classification/18_05/layer2_out_i8.hex",
              fc1_out_mem);
    build_pool2_input();

    for (int idx = 0; idx < POOL2_OUT_COUNT; idx++) begin
      host_write(1'b0, UB_ADDR_WIDTH'(idx), pool2_mem[idx]);
    end

    $display("  UB bank0[0:%0d] loaded with derived Pool2 tensor", POOL2_OUT_COUNT - 1);
    $display("  FC1 full layer: output count=%0d channel-major layer2_out_i8.hex",
             FC1_OUT_COUNT);
  endtask

  task automatic run_fc1_layer();
    int cycles;
    int output_pairs;

    print_header("TEST: V3 ROM layer controller -> full FC1 output vector");
    host_mode = 1'b0;
    ...
    while (!controller_done_o && !controller_error_o && (cycles < TIMEOUT_CYCLES)) begin
      ...
      if (vpu_data_valid_w) begin
        output_pairs++;
        vpu_output_count++;
        if ((output_pairs % 2) == 0) begin
          $display("  progress: output_pairs=%0d spatial=%0d oc_tile=%0d",
                   output_pairs, controller_spatial_idx_o, controller_oc_tile_o);
        end
      end
      ...
      if (accumulator_read_valid_w) begin
        ...
        if ((captured_acc_count % 2) == 0) begin
          $display("  ACC READ sample: spatial=%0d oc_tile=%0d data={lane1:%0d,lane0:%0d}",
                   controller_spatial_idx_o, controller_oc_tile_o,
    ...
    expect_flag("all FC1 oc tiles completed", controller_tile_count_o == 32'(FC1_OUTPUT_PAIRS));
    expect_flag("all FC1 VPU pairs observed", vpu_output_count == FC1_OUTPUT_PAIRS);
    expect_flag("all FC1 accumulator reads observed", captured_acc_count == FC1_OUTPUT_PAIRS);
  endtask
  ...
  task automatic check_output_buffer();
    ...
    print_header("TEST: Host readback full FC1 output vector");
    host_mode = 1'b1;
    mismatches = 0;

    for (int idx = 0; idx < FC1_OUT_COUNT; idx++) begin
      addr = OUT_BASE + UB_ADDR_WIDTH'(idx);
      expected = fc1_out_mem[idx];
      host_read(1'b1, addr, actual);

      expect_i8_eq($sformatf("fc1_out[%0d]", idx), actual, expected);
      if (actual !== expected) begin
        mismatches++;
      end
    end

    $display("  fc1 = {%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d}",
             $signed(fc1_out_mem[0]), $signed(fc1_out_mem[1]),
             $signed(fc1_out_mem[2]), $signed(fc1_out_mem[3]),
             $signed(fc1_out_mem[4]), $signed(fc1_out_mem[5]),
             $signed(fc1_out_mem[6]), $signed(fc1_out_mem[7]),
             $signed(fc1_out_mem[8]), $signed(fc1_out_mem[9]),
             $signed(fc1_out_mem[10]), $signed(fc1_out_mem[11]),
             $signed(fc1_out_mem[12]), $signed(fc1_out_mem[13]),
             $signed(fc1_out_mem[14]), $signed(fc1_out_mem[15]));
  ...
  initial begin
    ...
    $display("test_controller_v3_fc1_layer: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_v3_fc1_layer failed");
    end
    $finish;
  end

endmodule : test_controller_v3_fc1_layer
