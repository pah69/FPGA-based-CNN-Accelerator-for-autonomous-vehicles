...
module test_controller_v3_rom_layer_fc2;
  ...
  logic signed [DATA_WIDTH-1:0] expected_logits[0:9];
  ...
  task automatic init_signals();
    ...
    pass_count = 0;
    fail_count = 0;
    expected_logits[0] = -8'sd11;
    expected_logits[1] = -8'sd30;
    expected_logits[2] = 8'sd22;
    expected_logits[3] = 8'sd23;
    expected_logits[4] = -8'sd40;
    expected_logits[5] = -8'sd26;
    expected_logits[6] = -8'sd113;
    expected_logits[7] = 8'sd87;
    expected_logits[8] = 8'sd9;
    expected_logits[9] = 8'sd13;
  endtask
  ...
  task automatic run_v3_fc2_layer();
    int cycles;

    print_header("TEST: V3 ROM layer controller -> full FC2 logits");
    host_mode = 1'b0;
    ...
    while (!controller_done_o && !controller_error_o && (cycles < TIMEOUT_CYCLES)) begin
      ...
      if (wgt_req_valid_w && wgt_req_ready_w) begin
        $display("  WGT REQ: oc_tile=%0d req_k_tile=%0d active_k_tile=%0d",
                 controller_oc_tile_o, int'(wgt_req_tag_w - 16'h3000), controller_k_tile_o);
      end
      if (act_req_valid_w && act_req_ready_w) begin
        $display("  ACT REQ: oc_tile=%0d k_tile=%0d",
                 controller_oc_tile_o, int'(act_req_tag_w - 16'h4000));
      end
      if (act_launch_w && act_launch_ready_w) begin
        $display("  ACT LAUNCH: oc_tile=%0d k_tile=%0d", controller_oc_tile_o, controller_k_tile_o);
      end
    ...
    expect_flag("all FC2 oc tiles completed", controller_tile_count_o == 32'd5);
  endtask

  task automatic check_logits();
    logic signed [DATA_WIDTH-1:0] actual;
    int mismatches;

    print_header("TEST: Host readback V3 FC2 final logits");
    host_mode = 1'b1;
    mismatches = 0;
    ...
    for (int idx = 0; idx < 10; idx++) begin
      ...
      expect_i8_eq($sformatf("fc2 logit[%0d]", idx), actual, expected_logits[idx]);
  ...
  initial begin
    ...
    $display("test_controller_v3_rom_layer_fc2: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_v3_rom_layer_fc2 failed");
    end
    $finish;
  end

endmodule : test_controller_v3_rom_layer_fc2
