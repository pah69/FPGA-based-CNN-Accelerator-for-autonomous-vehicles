...
module test_controller_v3_rom_tile;
  ...
  localparam int TIMEOUT_CYCLES = 1600;
  ...
  logic signed [DATA_WIDTH-1:0] fc1_vector_mem[0:15];
  ...
  task automatic preload_fc1_vector();
    ...
    for (int idx = 0; idx < 16; idx++) begin
      host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(idx), fc1_vector_mem[idx]);
    end

    $display("  UB bank0[%0d:%0d] loaded from layer2_out_i8.hex", ACT_BASE, ACT_BASE + 15);
    $display("  FC2 oc_tile=4 should produce logits {class9:13,class8:9}");
  endtask

  task automatic run_rom_v3_tile();
    int cycles;

    print_header("TEST: ROM descriptor/params -> V3 tile controller -> datapath");
    host_mode = 1'b0;
    ...
    while (!controller_done_o && !controller_error_o && (cycles < TIMEOUT_CYCLES)) begin
      ...
      if (wgt_req_valid_w && wgt_req_ready_w) begin
        $display("  WGT REQ: req_k_tile=%0d active_k_tile=%0d tag=0x%04h",
                 int'(wgt_req_tag_w - 16'h3000), controller_k_tile_o, wgt_req_tag_w);
      end
      ...
      if (accumulator_row_done_w) begin
        $display("  ACC DONE: row=%0d ready=%b",
                 accumulator_row_done_addr_w, accumulator_row_ready_w[accumulator_row_done_addr_w]);
      end
      ...
      if (ctrl_ub_wr_en_w) begin
        $display("  UB WRITE: bank%0d[%0d] = %0d",
                 ctrl_ub_wr_bank_w, ctrl_ub_wr_addr_w, $signed(ctrl_ub_wr_data_w));
      end
    end

    $display("  CTRL   : done=%b error=%b state=%0d tile_state=%0d k_tile=%0d cycles=%0d err=0x%08h",
  ...
  task automatic check_output_buffer();
    ...
    print_header("TEST: Host readback V3 ROM-driven FC2 tile");
    host_mode = 1'b1;
    ...
    expect_i8_eq("fc2 class8 output", lane0, 8'sd9);
    expect_i8_eq("fc2 class9 output", lane1, 8'sd13);
  endtask
  ...
  initial begin
    ...
    expect_i8_eq("controller final state idle", DATA_WIDTH'(controller_state_o), DATA_WIDTH'(0));
    expect_i8_eq("activation vectors pushed", DATA_WIDTH'(dbg_act_vectors_pushed_w), DATA_WIDTH'(8));
    expect_i8_eq("weight reuse count", DATA_WIDTH'(dbg_weight_reuse_count_w), DATA_WIDTH'(8));

    $display("");
    $display("DEBUG COUNTERS");
    $display("  ctrl_cycles=%0d final_state=%0d final_k_tile=%0d",
             controller_cycle_count_o, controller_state_o, controller_k_tile_o);
    ...
    $display("test_controller_v3_rom_tile: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_v3_rom_tile failed");
    end
    $finish;
  end

endmodule : test_controller_v3_rom_tile
