...
module test_controller_v3_conv1_layer;
  ...
  task automatic run_conv1_layer();
    ...
    while (!controller_done_o && !controller_error_o && (cycles < TIMEOUT_CYCLES)) begin
      ...
      if (accumulator_read_valid_w) begin
        ...
        if ((controller_spatial_idx_o == CONV1_SPATIAL) && (controller_oc_tile_o == 16'd0)) begin
          $display("  ACC READ sample: spatial=%0d oc_tile=0 data={lane1:%0d,lane0:%0d}",
                   controller_spatial_idx_o,
    ...
    expect_flag("all Conv1 oc/spatial tiles completed", controller_tile_count_o == 32'(CONV1_NUM_SPATIAL * (CONV1_OUT_CH / SIZE)));
    expect_flag("all Conv1 VPU pairs observed", vpu_output_count == (CONV1_OUT_COUNT / SIZE));
    expect_flag("all Conv1 accumulator reads observed", captured_acc_count == (CONV1_OUT_COUNT / SIZE));
  endtask
  ...
  task automatic check_output_buffer();
    ...
    for (int idx = 0; idx < CONV1_OUT_COUNT; idx++) begin
      ...
      expected = conv1_out_mem[idx];
      host_read(1'b1, addr, actual);

      expect_i8_eq($sformatf("conv1_out[%0d]", idx), actual, expected);
      if (actual !== expected) begin
        mismatches++;
      end
    end

    $display("  samples: out[0]=%0d out[%0d]=%0d out[%0d]=%0d",
             $signed(conv1_out_mem[0]),
             int'(CONV1_SPATIAL),
             $signed(conv1_out_mem[CONV1_SPATIAL]),
             CONV1_OUT_COUNT - 1,
             $signed(conv1_out_mem[CONV1_OUT_COUNT - 1]));
  ...
  initial begin
    ...
    $display("test_controller_v3_conv1_layer: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_v3_conv1_layer failed");
    end
    $finish;
  end

endmodule : test_controller_v3_conv1_layer
