...
module test_maxpool2d_pool2;
  ...
  logic signed [DATA_WIDTH-1:0] expected_pool_mem[0:POOL2_OUT_COUNT-1];
  ...
  task automatic build_pool2_expected();
    int in_base;
    int out_idx;
  ...
  task automatic preload_conv2_output();
    print_header("TEST: Host preload Conv2 output tensor");
    host_mode = 1'b1;
    $readmemh("../../../CNN_model/python/mnist_classification/18_05/layer1_out_i8.hex",
              conv2_out_mem);
    build_pool2_expected();
    for (int idx = 0; idx < CONV2_OUT_COUNT; idx++) begin
      host_write(1'b1, UB_ADDR_WIDTH'(idx), conv2_out_mem[idx]);
    end
    $display("  UB bank1[0:%0d] loaded from layer1_out_i8.hex", CONV2_OUT_COUNT - 1);
    $display("  Pool2 expected tensor generated: 10x5x5 channel-major");
  endtask

  task automatic run_pool2();
    int cycles;

    print_header("TEST: MaxPool2d Pool2 pass");
    host_mode = 1'b0;
    ...
    expect_flag("pool wrote all outputs", write_count == POOL2_OUT_COUNT);
  endtask

  task automatic check_pool2_output();
    logic signed [DATA_WIDTH-1:0] actual;
    logic signed [DATA_WIDTH-1:0] sample0;
    logic signed [DATA_WIDTH-1:0] sample_last;
    int mismatch_count;

    print_header("TEST: Host readback Pool2 output tensor");
    host_mode = 1'b1;
    mismatch_count = 0;
    sample0 = '0;
    sample_last = '0;
    ...
    for (int idx = 0; idx < POOL2_OUT_COUNT; idx++) begin
      ...
      if (actual === expected_pool_mem[idx]) begin
        ...
        if (mismatch_count <= 8) begin
          $display("FAIL: pool2_out[%0d] got=%0d expected=%0d",
      ...
      if (idx == 0) begin
        sample0 = actual;
      end else if (idx == (POOL2_OUT_COUNT - 1)) begin
        sample_last = actual;
      end
    end

    $display("  samples: pool2[0]=%0d pool2[%0d]=%0d",
             $signed(sample0), POOL2_OUT_COUNT - 1, $signed(sample_last));
  ...
  initial begin
    ...
    preload_conv2_output();
    run_pool2();
    check_pool2_output();

    $display("test_maxpool2d_pool2: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_maxpool2d_pool2 failed");
    end
    $finish;
  end

endmodule : test_maxpool2d_pool2
