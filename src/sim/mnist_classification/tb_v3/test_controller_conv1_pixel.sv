...
module test_controller_conv1_pixel;
  ...
  localparam logic [UB_ADDR_WIDTH-1:0] OUT_ADDR0 =
      OUT_BASE + UB_ADDR_WIDTH'(CONV1_SPATIAL);
  localparam logic [UB_ADDR_WIDTH-1:0] OUT_ADDR1 =
      OUT_BASE + UB_ADDR_WIDTH'(CONV1_NUM_SPATIAL) + UB_ADDR_WIDTH'(CONV1_SPATIAL);
  ...
  task automatic preload_input_image();
    ...
    $display("  Conv1 pixel: spatial=%0d oc_tile=0 expected acc={lane1:19653,lane0:9485}",
             CONV1_SPATIAL);
  endtask
  ...
  task automatic run_conv1_pixel();
    ...
    print_header("TEST: ROM K-loop controller -> Conv1 single pixel");
    host_mode = 1'b0;
    ...
    expect_flag("accumulator read checked", captured_acc_count == 1);
  endtask
  ...
  task automatic check_output_buffer();
    ...
    print_header("TEST: Host readback Conv1 single pixel");
    host_mode = 1'b1;
    host_read(1'b1, OUT_ADDR0, lane0);
    host_read(1'b1, OUT_ADDR1, lane1);

    $display("  UB bank1[oc0@%0d,oc1@%0d] = {oc1:%0d,oc0:%0d}",
             OUT_ADDR0, OUT_ADDR1, $signed(lane1), $signed(lane0));
    expect_i8_eq("conv1 out oc0", lane0, 8'sd6);
    expect_i8_eq("conv1 out oc1", lane1, 8'sd14);
  endtask
  ...
  initial begin
    ...
    $display("test_controller_conv1_pixel: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_conv1_pixel failed");
    end
    $finish;
  end

endmodule : test_controller_conv1_pixel
