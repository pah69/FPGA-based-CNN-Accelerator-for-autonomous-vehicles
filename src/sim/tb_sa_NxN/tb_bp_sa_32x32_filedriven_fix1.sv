...
module tb_bp_sa_32x32_filedriven;
  ...
  localparam int DRAIN_CYCLES = 260;
  ...
  //     if (psum_valid_o[c]) begin
  //       logic signed [PSUM_WIDTH-1:0] psum_now;
  //       psum_now = $signed(psum_flatten_o[(c*PSUM_WIDTH)+:PSUM_WIDTH]);

  //       if (skip_count[c] < 1) begin
  //         // $display("[%0t] OUT-SKIP col=%0d val=%0d", $time, c, psum_now);
  //         skip_count[c] <= skip_count[c] + 1;
  //       end
  //       else if (out_count[c] < ACT_ROWS) begin
  //         got_mat[out_count[c]][c] <= psum_now;
  //         $display("[%0t] OUT col=%0d row=%0d val=%0d", $time, c, out_count[c], psum_now);
  //         out_count[c] <= out_count[c] + 1;
  //       end
  //       else begin
  //         $display("[%0t] OUT-DROP col=%0d val=%0d", $time, c, psum_now);
  //       end
  //     end
  //   end
  // end
  ...
  for (int c = 0; c < SIZE; c++) begin
    if (psum_valid_o[c]) begin
      ...
      if (out_count[c] < ACT_ROWS) begin
        got_mat[out_count[c]][c] <= psum_now;
        $display("[%0t] OUT col=%0d row=%0d val=%0d",
                 $time, c, out_count[c], psum_now);
        out_count[c] <= out_count[c] + 1;
      end
      else begin
        $display("[%0t] OUT-DROP col=%0d val=%0d",
                 $time, c, psum_now);
      end
    end
  end
