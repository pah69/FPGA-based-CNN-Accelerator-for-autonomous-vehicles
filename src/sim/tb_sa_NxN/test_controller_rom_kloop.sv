...
module test_controller_rom_kloop;
  ...
  localparam int TIMEOUT_CYCLES = 1200;
  ...
  task automatic preload_fc1_golden_vector();
    ...
    $display("  FC2 oc_tile=%0d should produce logits {class9:13,class8:9}", oc_tile_idx_i);
  endtask

  task automatic run_kloop_controller_datapath();
    int cycles;

    print_header("TEST: ROM K-loop controller -> UB -> TPU datapath");
    host_mode = 1'b0;
    ...
    while (!controller_done_o && !controller_error_o && (cycles < TIMEOUT_CYCLES)) begin
      ...
      if (accumulator_row_done_w) begin
        $display("  ACC DONE: row=%0d ready=%b accumulated_tiles=%0d",
                 accumulator_row_done_addr_w,
    ...
    $display("  CTRL   : done=%b error=%b state=%0d k_tile=%0d cycles=%0d err=0x%08h",
             controller_done_o, controller_error_o, controller_state_o,
             controller_k_tile_o, controller_cycle_count_o, controller_error_code_o);
  ...
  task automatic check_output_buffer();
    ...
    print_header("TEST: Host readback full-K FC2 logits");
    host_mode = 1'b1;
    host_read(1'b1, FC2_OUT0_ADDR, lane0);
    host_read(1'b1, FC2_OUT1_ADDR, lane1);

    $display("  UB bank1[%0d:%0d] = {class9:%0d,class8:%0d}",
             FC2_OUT0_ADDR, FC2_OUT1_ADDR, $signed(lane1), $signed(lane0));
    expect_i8_eq("fc2 class8 logit", lane0, 8'sd9);
    expect_i8_eq("fc2 class9 logit", lane1, 8'sd13);
  endtask
  ...
  initial begin
    ...
    run_kloop_controller_datapath();
    check_output_buffer();

    $display("test_controller_rom_kloop: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_rom_kloop failed");
    end
    $finish;
  end

endmodule : test_controller_rom_kloop
