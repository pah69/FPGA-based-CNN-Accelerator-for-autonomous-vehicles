...
module test_animals10_conv5_conv6_pool3_gap_fc1_fc2_case0;
    ...
    localparam int FINAL_STAGE_WEIGHT_COUNT =
        CONV5_WEIGHT_COUNT + CONV6_WEIGHT_COUNT + FC1_WEIGHT_COUNT + FC2_WEIGHT_COUNT;
    localparam int MODEL_WEIGHT_COUNT = 304224;
    localparam int MODEL_PARAM_COUNT = 586;
    localparam int FINAL_STAGE_PARAM_COUNT = 256 + 128 + 10;
    localparam int CONV5_WEIGHT_OFFSET = 65376;
    localparam int CONV5_PARAM_OFFSET = 192;
    localparam int FC2_OUT_COUNT = 10;
    localparam int TIMEOUT_CYCLES = 40010000;
    ...
    animals10_conv5_conv6_pool3_gap_fc1_fc2_i8 dut (
        .clk(clk),
        .rst_n(rst_n),
        .start_i(start),
        .busy_o(busy),
    ...
    task automatic load_dut_memories;
        begin
            ...
            for (int i = 0; i < FINAL_STAGE_WEIGHT_COUNT; i++) begin
                write_weight(i, weight_mem[CONV5_WEIGHT_OFFSET + i]);
            end
            for (int i = 0; i < FINAL_STAGE_PARAM_COUNT; i++) begin
                write_param(
                    i,
        ...
        begin
            ...
            do begin
                ...
                if (wait_cycles > TIMEOUT_CYCLES) begin
                    $fatal(1,
                           "Timeout waiting for final-stage completion after %0d cycles",
                           wait_cycles);
                end
            ...
            $display(
                "ANIMALS10_FINAL_STAGE outputs_seen=%0d acc_mismatches=%0d out_mismatches=%0d duplicates=%0d",
                outputs_seen, acc_mismatches, out_mismatches, duplicate_outputs
            );
            $display(
                "ANIMALS10_FINAL_STAGE counters total=%0d conv5_cycles=%0d conv5_tiles=%0d conv5_outputs=%0d conv6_cycles=%0d conv6_tiles=%0d conv6_outputs=%0d pool3_cycles=%0d pool3_outputs=%0d gap_cycles=%0d gap_outputs=%0d fc1_cycles=%0d fc1_outputs=%0d fc2_cycles=%0d fc2_outputs=%0d",
            ...
            if (duplicate_outputs != 0 || acc_mismatches != 0 || out_mismatches != 0) begin
                $fatal(1,
                       "Animals-10 final-stage DUT did not match Python INT8 golden output");
            end

            $display("PASS: Animals-10 final-stage DUT matches Python INT8 golden output");
        end
    endtask
    ...
    initial begin
        ...
        $display("Animals-10 final stage Conv5+Conv6+Pool3+GAP+FC1+FC2 DUT bench");
