...
module test_animals10_fc2_case0;
    ...
    localparam int FC2_WEIGHT_OFFSET = 302944;
    localparam int FC2_PARAM_OFFSET = 576;
    ...
    task automatic load_dut_memories;
        begin
            ...
            for (int i = 0; i < FC2_WEIGHT_COUNT; i++) begin
                write_weight(i, weight_mem[FC2_WEIGHT_OFFSET + i]);
            end
            for (int i = 0; i < FC2_OUT_COUNT; i++) begin
                write_param(
                    i,
                    bias_mem[FC2_PARAM_OFFSET + i],
                    requant_mult_mem[FC2_PARAM_OFFSET + i],
                    requant_shift_mem[FC2_PARAM_OFFSET + i]
                );
            end
        end
    endtask
        ...
        begin
            ...
            do begin
                ...
                if (wait_cycles > TIMEOUT_CYCLES) begin
                    $fatal(1, "Timeout waiting for FC2 completion after %0d cycles", wait_cycles);
                end
            ...
            $display(
                "ANIMALS10_FC2 outputs_seen=%0d acc_mismatches=%0d out_mismatches=%0d duplicates=%0d",
                outputs_seen, acc_mismatches, out_mismatches, duplicate_outputs
            );
            $display(
                "ANIMALS10_FC2 counters cycles=%0d outputs=%0d",
            ...
            if (outputs_seen != FC2_OUT_COUNT) begin
                $fatal(1, "Expected %0d FC2 outputs, got %0d", FC2_OUT_COUNT, outputs_seen);
            end
            if (duplicate_outputs != 0 || acc_mismatches != 0 || out_mismatches != 0) begin
                $fatal(1, "Animals-10 FC2 DUT did not match Python INT8 golden output");
            end

            $display("PASS: Animals-10 FC2 DUT matches Python INT8 golden output");
        end
    endtask
    ...
    initial begin
        ...
        input_hex = $sformatf("%s/debug_case0/fc1_out_i8.hex", export_dir);
        ...
        $display("Animals-10 FC2 DUT bench");
