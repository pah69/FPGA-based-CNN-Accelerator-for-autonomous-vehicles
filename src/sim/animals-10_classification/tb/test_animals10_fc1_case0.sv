...
module test_animals10_fc1_case0;
    ...
    localparam int FC1_OUT_COUNT = 128;
    ...
    logic signed [31:0] golden_acc[0:FC1_OUT_COUNT-1];
    logic signed [7:0] golden_out[0:FC1_OUT_COUNT-1];
    bit seen_output[0:FC1_OUT_COUNT-1];
    ...
    task automatic load_dut_memories;
        begin
            ...
            for (int i = 0; i < FC1_WEIGHT_COUNT; i++) begin
                write_weight(i, weight_mem[FC1_WEIGHT_OFFSET + i]);
            end
            for (int i = 0; i < FC1_OUT_COUNT; i++) begin
                write_param(
                    i,
        ...
        begin
            ...
            do begin
                ...
                if (wait_cycles > TIMEOUT_CYCLES) begin
                    $fatal(1, "Timeout waiting for FC1 completion after %0d cycles", wait_cycles);
                end

                if (output_valid) begin
                    idx = int'(output_index);
                    outputs_seen++;
                    if (idx < 0 || idx >= FC1_OUT_COUNT) begin
                        $fatal(1, "FC1 output index out of range: %0d", idx);
                    end
            ...
            $display(
                "ANIMALS10_FC1 outputs_seen=%0d acc_mismatches=%0d out_mismatches=%0d duplicates=%0d",
                outputs_seen, acc_mismatches, out_mismatches, duplicate_outputs
            );
            $display(
                "ANIMALS10_FC1 counters cycles=%0d outputs=%0d",
            ...
            if (outputs_seen != FC1_OUT_COUNT) begin
                $fatal(1, "Expected %0d FC1 outputs, got %0d", FC1_OUT_COUNT, outputs_seen);
            end
            if (duplicate_outputs != 0 || acc_mismatches != 0 || out_mismatches != 0) begin
                $fatal(1, "Animals-10 FC1 DUT did not match Python INT8 golden output");
            end

            $display("PASS: Animals-10 FC1 DUT matches Python INT8 golden output");
        end
    endtask
    ...
    initial begin
        ...
        input_hex = $sformatf("%s/debug_case0/gap_out_i8.hex", export_dir);
        ...
        golden_acc_hex = $sformatf("%s/debug_case0/fc1_acc_i32.hex", export_dir);
        golden_out_hex = $sformatf("%s/debug_case0/fc1_out_i8.hex", export_dir);

        $display("Animals-10 FC1 DUT bench");
