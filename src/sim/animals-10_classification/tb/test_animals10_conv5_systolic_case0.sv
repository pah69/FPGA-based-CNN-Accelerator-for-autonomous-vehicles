...
module test_animals10_conv5_systolic_case0;
    ...
    logic signed [31:0] golden_acc[0:CONV5_OUT_COUNT-1];
    logic signed [7:0] golden_out[0:CONV5_OUT_COUNT-1];
    bit seen_output[0:CONV5_OUT_COUNT-1];
    ...
    animals10_conv5_systolic_4x4 dut (
        .clk(clk),
        .rst_n(rst_n),
        .start_i(start),
        .busy_o(busy),
    ...
    task automatic load_dut_memories;
        begin
            ...
            for (int i = 0; i < CONV5_WEIGHT_COUNT; i++) begin
    ...
    task automatic check_stream;
        ...
        begin
            ...
            wait_cycles = 0;
            for (int i = 0; i < CONV5_OUT_COUNT; i++) begin
                seen_output[i] = 1'b0;
            end
            ...
            do begin
                ...
                if (wait_cycles > TIMEOUT_CYCLES) begin
                    $fatal(1, "Timeout waiting for systolic Conv5 completion after %0d cycles", wait_cycles);
                end

                if (output_valid) begin
                    idx = int'(output_index);
                    outputs_seen++;
                    if (idx < 0 || idx >= CONV5_OUT_COUNT) begin
            ...
            $display(
                "ANIMALS10_CONV5_SYSTOLIC outputs_seen=%0d acc_mismatches=%0d out_mismatches=%0d duplicates=%0d",
                outputs_seen, acc_mismatches, out_mismatches, duplicate_outputs
            );
            $display(
                "ANIMALS10_CONV5_SYSTOLIC counters cycles=%0d tiles=%0d outputs=%0d",
            ...
            if (outputs_seen != CONV5_OUT_COUNT) begin
                $fatal(1, "Expected %0d Conv5 outputs, got %0d", CONV5_OUT_COUNT, outputs_seen);
            end
            if (duplicate_outputs != 0 || acc_mismatches != 0 || out_mismatches != 0) begin
                $fatal(1, "Animals-10 Conv5 systolic DUT did not match Python INT8 golden output");
            end

            $display("PASS: Animals-10 Conv5 systolic DUT matches Python INT8 golden output");
        end
    endtask
    ...
    initial begin
        ...
        golden_acc_hex = $sformatf("%s/debug_case0/conv5_acc_i32.hex", export_dir);
        golden_out_hex = $sformatf("%s/debug_case0/conv5_out_i8.hex", export_dir);

        $display("Animals-10 Conv5 4x4 systolic DUT bench");
