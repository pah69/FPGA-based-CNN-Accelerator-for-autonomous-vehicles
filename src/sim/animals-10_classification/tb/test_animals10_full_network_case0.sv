...
module test_animals10_full_network_case0;
        ...
        begin
            ...
            do begin
                ...
                if (wait_cycles > TIMEOUT_CYCLES) begin
                    $fatal(1, "Timeout waiting for full-network completion after %0d cycles",
                           wait_cycles);
                end
            ...
            $display(
                "ANIMALS10_FULL_NETWORK outputs_seen=%0d acc_mismatches=%0d out_mismatches=%0d duplicates=%0d",
                outputs_seen, acc_mismatches, out_mismatches, duplicate_outputs
            );
            $display(
                "ANIMALS10_FULL_NETWORK counters total=%0d stage1=%0d stage2=%0d stage3=%0d conv1_cycles=%0d conv1_tiles=%0d conv1_outputs=%0d conv2_cycles=%0d conv2_tiles=%0d conv2_outputs=%0d pool1_cycles=%0d pool1_outputs=%0d conv3_cycles=%0d conv3_tiles=%0d conv3_outputs=%0d conv4_cycles=%0d conv4_tiles=%0d conv4_outputs=%0d pool2_cycles=%0d pool2_outputs=%0d conv5_cycles=%0d conv5_tiles=%0d conv5_outputs=%0d conv6_cycles=%0d conv6_tiles=%0d conv6_outputs=%0d pool3_cycles=%0d pool3_outputs=%0d gap_cycles=%0d gap_outputs=%0d fc1_cycles=%0d fc1_outputs=%0d fc2_cycles=%0d fc2_outputs=%0d",
                debug_total_cycle_count,
                debug_stage1_cycle_count,
                debug_stage2_cycle_count,
                debug_stage3_cycle_count,
            ...
            if (duplicate_outputs != 0 || acc_mismatches != 0 || out_mismatches != 0) begin
                $fatal(1, "Animals-10 full-network DUT did not match Python INT8 golden output");
            end

            $display("PASS: Animals-10 full-network DUT matches Python INT8 golden output");
        end
    endtask
    ...
    initial begin
        ...
        input_hex = $sformatf("%s/debug_case0/layer_00_input_i8.hex", export_dir);
        ...
        golden_out_hex = $sformatf("%s/debug_case0/final_logits_i8.hex", export_dir);

        $display("Animals-10 full-network case0 DUT bench");
