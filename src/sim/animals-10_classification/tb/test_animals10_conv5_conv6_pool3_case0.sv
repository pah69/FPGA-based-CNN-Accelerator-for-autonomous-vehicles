...
module test_animals10_conv5_conv6_pool3_case0;
    localparam int INPUT_COUNT = 64 * 16 * 16;
    localparam int CONV5_WEIGHT_COUNT = 128 * 64 * 3 * 3;
    localparam int CONV6_WEIGHT_COUNT = 128 * 128 * 3 * 3;
    localparam int CONV5_CONV6_WEIGHT_COUNT = CONV5_WEIGHT_COUNT + CONV6_WEIGHT_COUNT;
    localparam int MODEL_WEIGHT_COUNT = 304224;
    localparam int MODEL_PARAM_COUNT = 586;
    localparam int CONV5_CONV6_PARAM_COUNT = 256;
    localparam int CONV5_WEIGHT_OFFSET = 65376;
    localparam int CONV5_PARAM_OFFSET = 192;
    localparam int POOL3_OUT_COUNT = 128 * 8 * 8;
    ...
    logic signed [7:0] golden_pool3_out[0:POOL3_OUT_COUNT-1];
    bit seen_output[0:POOL3_OUT_COUNT-1];
    ...
    animals10_conv5_conv6_pool3_systolic_4x4 dut (
        .clk(clk),
        .rst_n(rst_n),
        .start_i(start),
        .busy_o(busy),
    ...
    task automatic load_hex_files;
        begin
            ...
            $readmemh(golden_pool3_hex, golden_pool3_out);
        end
    endtask
    ...
    task automatic load_dut_memories;
        begin
            ...
            for (int i = 0; i < CONV5_CONV6_WEIGHT_COUNT; i++) begin
                write_weight(i, weight_mem[CONV5_WEIGHT_OFFSET + i]);
            end
            for (int i = 0; i < CONV5_CONV6_PARAM_COUNT; i++) begin
                write_param(
                    i,
                    bias_mem[CONV5_PARAM_OFFSET + i],
                    requant_mult_mem[CONV5_PARAM_OFFSET + i],
                    requant_shift_mem[CONV5_PARAM_OFFSET + i]
                );
            end
        end
    endtask
    ...
    task automatic check_stream;
        ...
        begin
            ...
            for (int i = 0; i < POOL3_OUT_COUNT; i++) begin
                seen_output[i] = 1'b0;
            end
            ...
            do begin
                ...
                if (wait_cycles > TIMEOUT_CYCLES) begin
                    $fatal(1, "Timeout waiting for Conv5+Conv6+Pool3 completion after %0d cycles",
                           wait_cycles);
                end
                ...
                if (output_valid) begin
                    ...
                    outputs_seen++;
                    if (idx < 0 || idx >= POOL3_OUT_COUNT) begin
                        $fatal(1, "Pool3 output index out of range: %0d", idx);
                    end
                    ...
                    if (data !== golden_pool3_out[idx]) begin
                        out_mismatches++;
                        if (first_out_mismatch < 0) begin
                            first_out_mismatch = idx;
                            first_out_got = data;
                            first_out_expected = golden_pool3_out[idx];
                        end
                    end
                end
            end while (!done);
            ...
            $display(
                "ANIMALS10_CONV5_CONV6_POOL3 outputs_seen=%0d out_mismatches=%0d duplicates=%0d",
                outputs_seen, out_mismatches, duplicate_outputs
            );
            $display(
                "ANIMALS10_CONV5_CONV6_POOL3 counters total=%0d conv5_cycles=%0d conv5_tiles=%0d conv5_outputs=%0d conv6_cycles=%0d conv6_tiles=%0d conv6_outputs=%0d pool3_cycles=%0d pool3_outputs=%0d",
                debug_total_cycle_count,
                debug_conv5_cycle_count,
                debug_conv5_tile_count,
                debug_conv5_output_count,
                debug_conv6_cycle_count,
                debug_conv6_tile_count,
                debug_conv6_output_count,
                debug_pool3_cycle_count,
                debug_pool3_output_count
            );
            ...
            if (outputs_seen != POOL3_OUT_COUNT) begin
                $fatal(1, "Expected %0d Pool3 outputs, got %0d", POOL3_OUT_COUNT, outputs_seen);
            end
            if (duplicate_outputs != 0 || out_mismatches != 0) begin
                $fatal(1, "Animals-10 Conv5+Conv6+Pool3 DUT did not match Python INT8 golden output");
            end

            $display("PASS: Animals-10 Conv5+Conv6+Pool3 DUT matches Python INT8 golden output");
        end
    endtask
    ...
    initial begin
        ...
        input_hex = $sformatf("%s/debug_case0/pool2_out_i8.hex", export_dir);
        ...
        golden_pool3_hex = $sformatf("%s/debug_case0/pool3_out_i8.hex", export_dir);

        $display("Animals-10 Conv5+Conv6+Pool3 4x4 systolic DUT bench");
