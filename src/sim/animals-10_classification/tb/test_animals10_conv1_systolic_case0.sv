...
module test_animals10_conv1_systolic_case0;
    ...
    localparam int TIMEOUT_CYCLES = 20000000;
    ...
    bit seen_output[0:CONV1_OUT_COUNT-1];
    ...
    animals10_conv1_systolic_4x4 dut (
        .clk(clk),
        .rst_n(rst_n),
        .start_i(start),
        .busy_o(busy),
    ...
    task automatic check_stream;
        ...
        begin
            ...
            wait_cycles = 0;
            for (int i = 0; i < CONV1_OUT_COUNT; i++) begin
                seen_output[i] = 1'b0;
            end
            ...
            do begin
                ...
                if (wait_cycles > TIMEOUT_CYCLES) begin
                    $fatal(1, "Timeout waiting for systolic Conv1 completion after %0d cycles", wait_cycles);
                end

                if (output_valid) begin
                    idx = int'(output_index);
                    outputs_seen++;
                    if (idx < 0 || idx >= CONV1_OUT_COUNT) begin
            ...
            $display(
                "ANIMALS10_CONV1_SYSTOLIC outputs_seen=%0d acc_mismatches=%0d out_mismatches=%0d duplicates=%0d",
                outputs_seen, acc_mismatches, out_mismatches, duplicate_outputs
            );
            $display(
                "ANIMALS10_CONV1_SYSTOLIC counters cycles=%0d tiles=%0d outputs=%0d",
            ...
            if (duplicate_outputs != 0 || acc_mismatches != 0 || out_mismatches != 0) begin
                $fatal(1, "Animals-10 Conv1 systolic DUT did not match Python INT8 golden output");
            end

            $display("PASS: Animals-10 Conv1 systolic DUT matches Python INT8 golden output");
        end
    endtask
    ...
    initial begin
        ...
        $display("Animals-10 Conv1 4x4 systolic DUT bench");
