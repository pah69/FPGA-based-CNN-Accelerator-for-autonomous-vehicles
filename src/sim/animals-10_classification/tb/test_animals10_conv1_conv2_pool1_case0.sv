...
module test_animals10_conv1_conv2_pool1_case0;
    localparam int INPUT_COUNT = 3 * 64 * 64;
    localparam int CONV1_WEIGHT_COUNT = 32 * 3 * 3 * 3;
    localparam int CONV2_WEIGHT_COUNT = 32 * 32 * 3 * 3;
    localparam int CONV1_CONV2_WEIGHT_COUNT = CONV1_WEIGHT_COUNT + CONV2_WEIGHT_COUNT;
    localparam int MODEL_WEIGHT_COUNT = 304224;
    localparam int MODEL_PARAM_COUNT = 586;
    localparam int CONV1_CONV2_PARAM_COUNT = 64;
    localparam int POOL1_OUT_COUNT = 32 * 32 * 32;
    localparam int TIMEOUT_CYCLES = 30000000;
    ...
    logic signed [7:0] golden_pool1_out[0:POOL1_OUT_COUNT-1];
    bit seen_output[0:POOL1_OUT_COUNT-1];
    ...
    animals10_conv1_conv2_pool1_systolic_4x4 dut (
        .clk(clk),
        .rst_n(rst_n),
        .start_i(start),
        .busy_o(busy),
    ...
    task automatic load_hex_files;
        begin
            ...
            $readmemh(golden_pool1_hex, golden_pool1_out);
        end
    endtask
    ...
    task automatic load_dut_memories;
        begin
            ...
            for (int i = 0; i < CONV1_CONV2_WEIGHT_COUNT; i++) begin
                write_weight(i, weight_mem[i]);
            end
            for (int i = 0; i < CONV1_CONV2_PARAM_COUNT; i++) begin
    ...
    task automatic check_stream;
        ...
        begin
            ...
            for (int i = 0; i < POOL1_OUT_COUNT; i++) begin
                seen_output[i] = 1'b0;
            end
            ...
            do begin
                ...
                if (wait_cycles > TIMEOUT_CYCLES) begin
                    $fatal(1, "Timeout waiting for Conv1+Conv2+Pool1 completion after %0d cycles",
                           wait_cycles);
                end
                ...
                if (output_valid) begin
                    ...
                    outputs_seen++;
                    if (idx < 0 || idx >= POOL1_OUT_COUNT) begin
                        $fatal(1, "Pool1 output index out of range: %0d", idx);
                    end
                    ...
                    if (data !== golden_pool1_out[idx]) begin
                        out_mismatches++;
                        if (first_out_mismatch < 0) begin
                            first_out_mismatch = idx;
                            first_out_got = data;
                            first_out_expected = golden_pool1_out[idx];
                        end
                    end
                end
            end while (!done);
            ...
            $display(
                "ANIMALS10_CONV1_CONV2_POOL1 outputs_seen=%0d out_mismatches=%0d duplicates=%0d",
                outputs_seen, out_mismatches, duplicate_outputs
            );
            $display(
                "ANIMALS10_CONV1_CONV2_POOL1 counters total=%0d conv1_cycles=%0d conv1_tiles=%0d conv1_outputs=%0d conv2_cycles=%0d conv2_tiles=%0d conv2_outputs=%0d pool1_cycles=%0d pool1_outputs=%0d",
                debug_total_cycle_count,
                debug_conv1_cycle_count,
                debug_conv1_tile_count,
                debug_conv1_output_count,
                debug_conv2_cycle_count,
                debug_conv2_tile_count,
                debug_conv2_output_count,
                debug_pool1_cycle_count,
                debug_pool1_output_count
            );
            ...
            if (outputs_seen != POOL1_OUT_COUNT) begin
                $fatal(1, "Expected %0d Pool1 outputs, got %0d", POOL1_OUT_COUNT, outputs_seen);
            end
            if (duplicate_outputs != 0 || out_mismatches != 0) begin
                $fatal(1, "Animals-10 Conv1+Conv2+Pool1 DUT did not match Python INT8 golden output");
            end

            $display("PASS: Animals-10 Conv1+Conv2+Pool1 DUT matches Python INT8 golden output");
        end
    endtask
    ...
    initial begin
        ...
        golden_pool1_hex = $sformatf("%s/debug_case0/pool1_out_i8.hex", export_dir);

        $display("Animals-10 Conv1+Conv2+Pool1 4x4 systolic DUT bench");
