...
module test_controller_rom_layer;
  ...
  localparam int TIMEOUT_CYCLES = 6000;
  ...
  task automatic preload_fc1_golden_vector();
    print_header("TEST: Host preload FC1 golden output vector");
    host_mode = 1'b1;
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(0),  8'sd0);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(1),  8'sd9);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(2),  8'sd85);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(3),  8'sd0);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(4),  8'sd0);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(5),  8'sd22);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(6),  8'sd0);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(7),  8'sd12);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(8),  8'sd0);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(9),  8'sd47);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(10), 8'sd0);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(11), 8'sd0);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(12), 8'sd41);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(13), 8'sd0);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(14), 8'sd56);
    host_write(1'b0, ACT_BASE + UB_ADDR_WIDTH'(15), 8'sd88);
    $display("  UB bank0[%0d:%0d] loaded from layer2_out_i8.hex", ACT_BASE, ACT_BASE + 15);
    $display("  FC2 layer should produce logits {-11,-30,22,23,-40,-26,-113,87,9,13}");
  endtask

  task automatic run_layer_controller_datapath();
    int cycles;

    print_header("TEST: ROM layer controller -> full FC2 logits");
    host_mode = 1'b0;
    ...
    while (!controller_done_o && !controller_error_o && (cycles < TIMEOUT_CYCLES)) begin
      ...
      if (accumulator_row_done_w) begin
        $display("  ACC DONE: oc_tile=%0d row=%0d ready=%b",
                 controller_oc_tile_o, accumulator_row_done_addr_w,
    ...
    $display("  CTRL   : done=%b error=%b state=%0d tile_state=%0d oc_tile=%0d k_tile=%0d cycles=%0d err=0x%08h",
             controller_done_o, controller_error_o, controller_state_o,
             controller_tile_state_o, controller_oc_tile_o, controller_k_tile_o,
  ...
  task automatic check_output_buffer();
    logic signed [DATA_WIDTH-1:0] logit[0:9];

    print_header("TEST: Host readback FC2 final logits");
    host_mode = 1'b1;
    for (int idx = 0; idx < 10; idx++) begin
      host_read(1'b1, OUT_BASE + UB_ADDR_WIDTH'(idx), logit[idx]);
    end

    $display("  logits = {%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d}",
             $signed(logit[0]), $signed(logit[1]), $signed(logit[2]), $signed(logit[3]),
             $signed(logit[4]), $signed(logit[5]), $signed(logit[6]), $signed(logit[7]),
             $signed(logit[8]), $signed(logit[9]));
    expect_i8_eq("fc2 logit class0", logit[0], -8'sd11);
    expect_i8_eq("fc2 logit class1", logit[1], -8'sd30);
    expect_i8_eq("fc2 logit class2", logit[2], 8'sd22);
    expect_i8_eq("fc2 logit class3", logit[3], 8'sd23);
    expect_i8_eq("fc2 logit class4", logit[4], -8'sd40);
    expect_i8_eq("fc2 logit class5", logit[5], -8'sd26);
    expect_i8_eq("fc2 logit class6", logit[6], -8'sd113);
    expect_i8_eq("fc2 logit class7", logit[7], 8'sd87);
    expect_i8_eq("fc2 logit class8", logit[8], 8'sd9);
    expect_i8_eq("fc2 logit class9", logit[9], 8'sd13);
  endtask
  ...
  initial begin
    ...
    preload_fc1_golden_vector();
    run_layer_controller_datapath();
    check_output_buffer();

    $display("test_controller_rom_layer: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_rom_layer failed");
    end
    $finish;
  end

endmodule : test_controller_rom_layer
