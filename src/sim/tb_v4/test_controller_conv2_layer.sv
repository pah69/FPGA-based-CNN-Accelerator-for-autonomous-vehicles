...
module test_controller_conv2_layer;
  ...
  logic signed [DATA_WIDTH-1:0] expected_out_mem[0:CONV2_OUT_COUNT-1];
  ...
  task automatic run_conv2_layer();
    ...
    print_header("TEST: ROM layer controller -> full Conv2 output tensor");
    host_mode = 1'b0;
    ...
    while (!controller_done_o && !controller_error_o && (cycles < TIMEOUT_CYCLES)) begin
      ...
      if (vpu_data_valid_w) begin
        output_pair_count++;
        if ((output_pair_count % 128) == 0) begin
    ...
    expect_flag("all Conv2 output pairs observed", output_pair_count == CONV2_OUTPUT_PAIRS);
  endtask

  task automatic check_conv2_output();
    logic signed [DATA_WIDTH-1:0] actual;
    logic signed [DATA_WIDTH-1:0] sample0;
    logic signed [DATA_WIDTH-1:0] sample120;
    logic signed [DATA_WIDTH-1:0] sample241;
    ...
    for (int idx = 0; idx < CONV2_OUT_COUNT; idx++) begin
      ...
      if (actual === expected_out_mem[idx]) begin
        ...
        if (mismatch_count <= 8) begin
          $display("FAIL: conv2_out[%0d] got=%0d expected=%0d",
      ...
      if (idx == 0) begin
        sample0 = actual;
      end else if (idx == 120) begin
        sample120 = actual;
      end else if (idx == 241) begin
        sample241 = actual;
      end else if (idx == (CONV2_OUT_COUNT - 1)) begin
        sample_last = actual;
      end
    end

    $display("  samples: out[0]=%0d out[120]=%0d out[241]=%0d out[%0d]=%0d",
             $signed(sample0), $signed(sample120), $signed(sample241),
             CONV2_OUT_COUNT - 1, $signed(sample_last));
  ...
  initial begin
    ...
    $display("test_controller_conv2_layer: checks=%0d pass=%0d fail=%0d",
             test_count, pass_count, fail_count);
    if (fail_count != 0) begin
      $fatal(1, "test_controller_conv2_layer failed");
    end
    $finish;
  end

endmodule : test_controller_conv2_layer
