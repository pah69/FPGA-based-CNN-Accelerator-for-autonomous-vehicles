...
// Correctness-first Animals-10 Conv1 engine using the SIZE=4 systolic array.
//
// This is the first real Conv1 datapath step after the direct reference block.
// It intentionally favors a simple, inspectable schedule over throughput:
//   - 4 output channels are computed at a time across the array columns.
//   - 3x3x3 = 27 Conv1 K values are processed as seven 4-row K tiles.
//   - Activations are row-skewed so the bottom psum for each column is one
//     4-element dot-product tile.
//   - Tile sums are accumulated locally, then bias/requant/ReLU is applied.
module animals10_conv1_systolic_4x4 #(
  ...
  initial begin : parameter_check
    assert (SIZE == 4) else $error("animals10_conv1_systolic_4x4 requires SIZE=4");
...
endmodule : animals10_conv1_systolic_4x4
