...
module datapath #(
    ...
    input logic                                fc_local_accum_mode_i,
  ...
  logic                                      fc_reducer_busy_w;
  logic                                      fc_reducer_write_active_w;
  logic signed [       (ACC_WIDTH*SIZE)-1:0] fc_reducer_read_flatten_w;
  logic                                      fc_reducer_read_valid_w;
  logic                                      fc_reducer_row_done_w;
  logic        [         ACC_ADDR_WIDTH-1:0] fc_reducer_row_done_addr_w;
  logic        [              ACC_DEPTH-1:0] fc_reducer_row_ready_w;

  logic signed [       (OUT_WIDTH*SIZE)-1:0] vpu_data_flatten_w;
  logic                                      vpu_data_valid_w;
  logic                                      vpu_done_w;
  logic                                      fc_local_accum_mode_w;

  assign fc_local_accum_mode_w = (fc_local_accum_mode_i === 1'b1);
  ...
  mxu #(
      ...
      .local_accum_enable_i(1'b0),
      .local_accum_clear_i (1'b0),
  ...
  psum_packer #(
      ...
      .capture_en_i         (accumulator_write_en_i && !fc_local_accum_mode_w),
  ...
  accumulator_array #(
      ...
      .write_en_i         (packed_psum_valid_w && !fc_local_accum_mode_w),
  ...
  fc_row_reducer #(
      ...
      .read_en_i          (accumulator_read_en_i),
      .read_addr_i        (accumulator_read_addr_i),
      .busy_o             (fc_reducer_busy_w),
      .write_active_o     (fc_reducer_write_active_w),
      .read_data_flatten_o(fc_reducer_read_flatten_w),
      .read_valid_o       (fc_reducer_read_valid_w),
      .row_done_o         (fc_reducer_row_done_w),
      .row_done_addr_o    (fc_reducer_row_done_addr_w),
      .row_ready_o        (fc_reducer_row_ready_w)
  );
  ...
  vector_processing_unit #(
      ...
      .rst_n                       (rst_n),
      .acc_flatten_i               (
          fc_local_accum_mode_w ? fc_reducer_read_flatten_w : accumulator_read_flatten_w
      ),
      .acc_valid_i                 (
          fc_local_accum_mode_w ? fc_reducer_read_valid_w : accumulator_read_valid_w
      ),
  ...
  assign psum_packer_busy_o          = fc_local_accum_mode_w ? fc_reducer_busy_w : psum_packer_busy_w;
  assign accumulator_write_active_o  =
      fc_local_accum_mode_w ? fc_reducer_write_active_w : packed_psum_valid_w;
  assign accumulator_read_flatten_o  =
      fc_local_accum_mode_w ? fc_reducer_read_flatten_w : accumulator_read_flatten_w;
  assign accumulator_read_valid_o    =
      fc_local_accum_mode_w ? fc_reducer_read_valid_w : accumulator_read_valid_w;
  assign accumulator_row_done_o      =
      fc_local_accum_mode_w ? fc_reducer_row_done_w : accumulator_row_done_w;
  assign accumulator_row_done_addr_o =
      fc_local_accum_mode_w ? fc_reducer_row_done_addr_w : accumulator_row_done_addr_w;
  assign accumulator_row_ready_o     =
      fc_local_accum_mode_w ? fc_reducer_row_ready_w : accumulator_row_ready_w;
